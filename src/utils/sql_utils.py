...
def create_table(mariadb_engine: sqlalchemy.engine, table_name: str, col_datatype_dct: dict, primary_key: str=None, index_lst: list=None, foreignkey_ref_dct: dict=None):
...
def df_to_sql_split(mariadb_engine: sqlalchemy.engine, df: pd.DataFrame, table_name: str, chunksize: int=50):
...
def get_latest_date_in_table(mariadb_engine: sqlalchemy.engine, table_name: str, date_col: str='date'):
    sql_query = f'SELECT MAX({date_col}) FROM {table_name}'
    (latest_date, ) = mariadb_engine.execute(sql_query).fetchone()
    if not latest_date:
...
def table_exists(db_engine: sqlalchemy.engine, schema_name: str, table_name: str):
    ...
    (exists_num, ) = db_engine.execute(sql_query).fetchone()
...
def several_updates_table(db_engine: sqlalchemy.engine, table_name: str, update_df: pd.DataFrame, index_df: pd.DataFrame):
    ...
    for i in range(len(index_df)):
        ...
        update_table(db_engine, table_name, update_dct, index_dct)
...
def table_empty(db_engine: sqlalchemy.engine, table_name: str):
    ...
    (empty_num, ) = db_engine.execute(sql_query).fetchone()
...
def col_dtypes(db_engine: sqlalchemy.engine, schema_name: str, table_name: str):
    ...
    sql_query = f"SELECT column_name, data_type FROM information_schema.columns where table_schema = '{schema_name}' and table_name='{table_name}'"
    res = db_engine.execute(sql_query).fetchall()
